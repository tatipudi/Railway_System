...
def book_ticket() :
    if not logged_in :
        ...
        trains[choice].print_seat_availabilty()
    ...
    if pnr in ticket_dict :
        ...
        if prompt == 'y ':
            if logged_in :
                    ...
                    trains[ticket_dict[pnr].train_num].seat[ticket_dict[pnr].coach] += ticket_dict[pnr].ticket_num
...
def check_seat_availabilty(flag = ' '):
    src = input("Enter the source destination : ")
    des = input("Enter the destination station : ")
    flag_2 = 0
    for i in trains :
        if trains[i].src == src and trains[i].des==des :
                print("Train Name : ",trains[i].name , " " , "Number", trains[i].num, " ", "Day of travel : ",train[i].day_of_travel )
                flag_2 +=1
        ...
        if flag == ' ' :
            ...
            trains[train_num].print_seat_availabilty()
            menu()
        else :
            pass
...
t3 = train('Banglore', 22353, '11:56', '03:12', 'ctc', 'ban', 'Fri', 33,24,77,455,325,533)
trains={t1.num:t1, t2.num:t2, t3.num:t3}
u1 = user(1111, 'Mouni','Cuttack', '77628302373', 'Mouni')
u2 = user(2322, 'Devi', 'Newyork', ' 7330646567', ' Devi')
...
def menu():
    ...
    print(" 3. Check PNR ")
    print(" 4. Check Seat Availabilty ")
